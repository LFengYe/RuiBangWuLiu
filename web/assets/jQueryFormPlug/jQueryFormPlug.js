...
$.fn.insertInputForm = function(options){
	var _default_ = {
		controls:{},                       //输入框描述
		mustWrite:[],                      //必填项目
		lastInputCallBack:function(){console.log("我是一个回调函数！")},    //最后一个输入框  对enter键的回调函数响应
	...
	var _pro_ = {
		getDOM:function(){
			...
			for(var i in controls ) {
				...
				if(controls[i].indexOf('@') >= 0) {
					...
					inputResult += "<div><span>" + txt + "</span>:<input class='input' type='date' name='" + i + "'/></div>";				
		...
		isFinishForm : function() {
			...
			for(var j = 0; j < options.mustWrite.length; j++) {
				...
				if($input.val() == '') {
//					$input.attr("data-tip-msg", "必填！！").tips({
//						msg: '必填'
//					}).trigger('alert');
...
//							$input.attr("data-tip-msg", txt + "不能超过" + maxLength + "个字节").tips({
//								msg: txt + "不能超过" + maxLength + "个字节"
//							}).trigger('alert');
...
//							$input.attr("data-tip-msg", "请输入数字类型").tips({
//								msg: '请输入长度不超过'+maxLength+'位数字'
//							}).trigger('alert');
					...
					case 'bool':
						if(val != "是" && val != "否" && val != '') {
//							$input.attr("data-tip-msg", "请输入是或否").tips({
//								msg: '请输入是或否'
//							}).trigger('alert');
                            alert('请输入是或否');
							$input.val('').trigger('focus');
       ...
       EvtInputFocus:function(){
       	...
       	this.$inputs.filter(".event").on('focus', function(){
       		...
       		var arr,txt,relys;
       		if(obj[name].indexOf("@table")>=0){     //进行ajax请求     返回的是table
			    ...
			    //进行ajax请求
       			options.requesFun(data,function(data){
					...
					//	$input.parent().next().children("input").trigger("focus");
       			...
       			arr = obj[name].split(',').slice(1).map(function(it){
       				return it.slice(1);
       			});
				if(arr.length == 1){
					//console.log("wc");
